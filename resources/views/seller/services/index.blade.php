...
<div class="content">
 ...
 <h2 class="text-2xl font-semibold mb-3 md:mb-0">Your Services</h2>

 <div class="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto">
 <form action="{{ route('seller.services.index') }}" method="GET" class="flex flex-1 sm:flex-none">
 <input
 type="text"
 name="search"
 value="{{ request('search') }}"
 placeholder="Search services..."
 class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100 rounded-l-xl"
 >
 <button class="inline-flex items-center justify-center rounded-xl px-3 py-2 text-sm font-semibold transition border border-emerald-600 bg-emerald-600 text-white hover:bg-emerald-700 rounded-r-xl ml-0">Search</button>
 </form>
 ...
 <i class="fas fa-image fa-2x text-slate-500"></i>
 </div>
 @endif
 {{-- Status Badge --}}
 @php
 ...
 @endphp
 <span class="absolute top-0 right-0 m-2 inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-semibold {{ $statusClasses[$service->status] ?? 'bg-slate-200 text-slate-700 border-slate-300' }}">
 ...
 {{-- Card Body --}}
 <div class="p-4">
 <h5 class="text-base font-bold text-slate-900 truncate" title="{{ $service->name }}">
 {{ $service->name }}
 </h5>
