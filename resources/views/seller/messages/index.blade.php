...
<div class="content">
 ...
 <div>
 <h1 class="text-xl font-semibold mb-1">Customer Conversations</h1>
 ...
 <i class="fa-solid fa-circle mr-1 text-[8px]"></i>{{ $unreadCount }} new message{{ $unreadCount > 1 ? 's' : '' }}
 </span>
 ...
 <i class="fa-regular fa-circle-check mr-1"></i>Mark All as Read
 </button>
 </form>
 </div>
 @endif
 ...
 <i class="fa-solid fa-xmark-circle mr-1"></i>Clear Product Filter
 </a>
 @endif
 ...
 <button type="submit" class="inline-flex items-center justify-center rounded-xl px-3 py-2 text-sm font-semibold transition border border-emerald-600 text-emerald-700 hover:bg-emerald-50 px-2.5 py-1.5 text-xs rounded-lg"><i class="fa-solid fa-magnifying-glass"></i> Search</button>
 @if(request('search'))
 ...
 <table class="min-w-full divide-y divide-slate-200 text-sm align-middle mb-0 conversation-table">
 ...
 <tr class="conversation-row {{ $conversation['unread_count'] > 0 ? 'bg-amber-50' : '' }}">
 ...
 @else
 <div class="bg-slate-50 border rounded mr-2 flex items-center justify-center" style="width:38px;height:38px;">
 <i class="fa-regular fa-comments text-slate-500"></i>
 </div>
 @endif
 @endif
 <div class="flex-1">
 @if($conversation['product'])
 <span class="font-semibold text-slate-900 truncate block product-name" style="max-width:120px;" title="{{ $conversation['product']->name ?? '-' }}">
 ...
 <a href="?product={{ $conversation['product']->id ?? '' }}" class="inline-flex items-center justify-center rounded-xl p-0 text-xs font-semibold text-emerald-700 hover:underline ml-1">All for this product</a>
 @else
 ...
 <i class="fa-regular fa-envelope mr-1"></i>{{ \Illuminate\Support\Str::limit($conversation['other_user']->email ?? '', 22) }}
 </span>
 ...
 <i class="fa-regular fa-clock mr-1"></i>{{ $conversation['latest_message']->created_at->diffForHumans() }}
 <span class="ml-2">
 <i class="fa-regular fa-comments mr-1"></i>{{ $conversation['total_messages'] }} message{{ $conversation['total_messages'] > 1 ? 's' : '' }}
 </span>
 </div>
 </td>
 <td class="text-center">
 @if($conversation['unread_count'] > 0)
 <span class="inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-semibold bg-amber-100 text-amber-800 border-amber-200 text-slate-900"><i class="fa-solid fa-circle"></i> Unread</span>
 @else
 <span class="inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-semibold bg-emerald-100 text-emerald-800 border-emerald-200"><i class="fa-solid fa-check"></i> Read</span>
 @endif
 </td>
 <td class="text-right">
 ...
 @csrf
 <button type="submit" class="inline-flex items-center justify-center rounded-xl px-3 py-2 text-sm font-semibold transition border border-slate-300 text-slate-700 hover:bg-slate-100 px-2.5 py-1.5 text-xs rounded-lg shadow-sm mark-unread-btn" title="Mark as Unread">
 <i class="fa-solid fa-circle"></i> Mark Unread
 </button>
 </form>
 @endif
 ...
 <i class="fa-solid fa-inbox mr-2"></i> No conversations found for your products.
 </td>
 </tr>
 @endforelse
 </tbody>
...
<style>
 ...
 .conversation-row.bg-amber-50 {
 background-color: #fff3cd !important;
 }
 .conversation-row.bg-amber-50:hover {
 ...
 .flex.gap-2 .mark-read-btn,
 .flex.gap-2 .mark-unread-btn,
 .flex.gap-2 .reply-btn {
 width: 100%;
 }
 }
</style>
