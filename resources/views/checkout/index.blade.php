...
@section('main')
<style>
  .form-control,.form-select{background:#fff;color:#000;border:1px solid #ced4da}
  .form-control:focus,.form-select:focus{background:#fff;color:#000;border-color:#80bdff;
     box-shadow:0 0 0 .2rem rgba(38,143,255,.25)}
  .text-danger{font-size:.875rem}
  @media(min-width:992px){.sticky-summary{position:sticky;top:100px;z-index:10}}
</style>
      ...
      <form action="{{ route('store_order') }}" method="POST" class="row gy-5 gy-lg-0" novalidate>
        ...
        <div class="col-lg-7 order-lg-1">
          ...
          <div class="row g-3">
            ...
            {{-- Billing fields (hidden until unchecked) --}}
        ...
        <div class="col-lg-5 order-lg-2">
          <div class="sticky-summary bg-white p-4 shadow-sm rounded">
              ...
              @foreach($cart as $row)
                ...
                <li class="list-group-item d-flex justify-content-between">
                  <div>
                    ...
                    @endif
                    <small>Ship: {{ $prof['name']??'Standard' }} ({{ $currency }} {{ number_format($rate,2) }})</small>
            ...
            <div class="d-flex justify-content-between mb-2">
              <span>Shipping</span>
              <strong>{{ $currency }} {{ number_format($shipping,2) }}</strong>
            </div>
            <hr>
            <div class="d-flex justify-content-between">
              <span><strong>Total</strong></span>
              <strong>{{ $currency }} {{ number_format($subtotal+$shipping,2) }}</strong>
            </div>

            <div class="d-grid mt-4">
              <button class="btn btn-success btn-lg" type="submit">Place Your Order</button>
            </div>
          </div>
        </div>
      </form>
...
<script>
function toggleBillingAddress(c){document.getElementById('billing_address_fields').style.display=c?'none':'block'}
</script>
@endsection
