...
<div class="content flex justify-center">
  <div class="rounded-2xl border border-slate-200 bg-white shadow-sm" style="max-width:480px;">
    <div class="border-b border-slate-200 px-4 py-3 bg-success text-white">
      <h4 class="mb-0 text-white">{{ $isFreePlan ? ($product->is_active ? 'Listing Activated!' : 'Plan Activated!') : 'Payment Received!' }}</h4>
    </div>
    <div class="p-4 sm:p-5">
      @if($isFreePlan)
        <p>Your <strong>{{ $planLabel ?? ucfirst($plan) }}</strong> plan has been activated with no payment due.</p>
      @else
