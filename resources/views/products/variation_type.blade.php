            ...
            <div class="divide-y divide-slate-200 rounded-xl border border-slate-200 mb-3">
              @forelse($type->options as $opt)
                <div class="px-4 py-3 flex justify-between items-center">
                  <form class="flex items-center flex-grow-1"
                  ...
                  <form action="{{ route('variationOptions.destroy', $opt) }}"
                    ...
                    @csrf
                    @method('DELETE')
                    <button class="inline-flex items-center justify-center rounded-xl px-4 py-2 text-sm font-semibold transition px-3 py-1.5 text-xs border border-rose-600 text-rose-700 hover:bg-rose-50" title="Delete">
