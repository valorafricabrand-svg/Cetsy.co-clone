        ...
        <div class="grid grid-cols-12 gap-4">
            ...
            <div class="col-span-12 lg:col-span-9">
                ...
                <div class="mb-4 rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
                    <h3 class="text-2xl font-semibold text-slate-900">{{ __('Dashboard') }}</h3>
                    <p class="mt-1 text-slate-500">{{ __('Welcome back, :name.', ['name' => Auth::user()->name]) }}</p>
                </div>
                    ...
                    <div class="col-span-12 md:col-span-4">
                        <a href="{{ route('account.orders') }}" class="block h-full rounded-2xl border border-slate-200 bg-white text-center shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                            <div class="flex flex-col items-center justify-center p-5">
                                ...
                                <h5 class="text-lg font-bold text-sky-700">{{ __('Orders') }}</h5>
                                <p class="mt-1 text-sm text-slate-500">{{ trans_choice('{1} :count Order|[2,*] :count Orders', $ordersCount, ['count' => $ordersCount]) }}</p>
                            </div>
                        </a>
                    </div>
                    <div class="col-span-12 md:col-span-4">
                        <a href="{{ route('wishlist') }}" class="block h-full rounded-2xl border border-slate-200 bg-white text-center shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                            <div class="flex flex-col items-center justify-center p-5">
                                ...
                                <p class="mt-1 text-sm text-slate-500">{{ trans_choice('{1} :count Item|[2,*] :count Items', $wishlistCount, ['count' => $wishlistCount]) }}</p>
                            </div>
                        </a>
                    </div>
                    <div class="col-span-12 md:col-span-4">
                        <a href="{{ route('wallet.index') }}" class="block h-full rounded-2xl border border-slate-200 bg-white text-center shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                            <div class="flex flex-col items-center justify-center p-5">
                                ...
                                <h5 class="text-lg font-bold text-amber-700">{{ __('Wallet') }}</h5>
                            ...
                            <ul class="divide-y divide-slate-200 rounded-xl border border-slate-200">
                                @foreach($recentOrders as $order)
                                    <li class="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                                        <div>
                                            <div class="text-sm font-semibold text-slate-900">{{ __('Order #:id', ['id' => $order->id]) }}</div>
                                            <div class="text-sm text-slate-500">{{ __('Date: :date', ['date' => $order->created_at->format('d M, Y')]) }}</div>
                                            <div class="text-sm text-slate-700">
                                                {{ __('Status: :status | Total: :total', ['status' => ucfirst($order->status), 'total' => money($order->total_amount ?? $order->total)]) }}
                                            </div>
                                        </div>
                                        <div class="flex flex-wrap gap-2">
                                            <a href="{{ route('buyer.orders.show', $order->id) }}" class="inline-flex items-center justify-center rounded-xl border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-50">{{ __('View Details') }}</a>
                                            @if($order->status === 'pending')
                                                <a href="{{ route('pay_now', $order->id) }}" class="inline-flex items-center justify-center rounded-xl bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-emerald-500">{{ __('Pay Now') }}</a>
                                            @endif
                                        </div>
                                    </li>
                                @endforeach
                ...
                <section class="rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div class="border-b border-slate-200 px-4 py-3">
                        <h4 class="text-xl font-semibold text-slate-900">{{ __('Recommended for You') }}</h4>
                    </div>
                    <div class="p-4">
                        @if($recommendedProducts->isEmpty())
                            <div class="rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-800" role="alert">
                                {{ __('No recommended products yet. Browse the marketplace to personalize your picks.') }}
                            </div>
                        @else
