                ...
                <div class="card-body small">
                    <div class="row gy-2">
                        <div class="col-12 col-md-6">
                            <span class="fw-semibold">Name:</span> {{ $order->shop->name }}
                        </div>

                        @if($order->shop->description)
                            <div class="col-12">
                                <span class="fw-semibold">Description:</span> {{ $order->shop->description }}
                            </div>
                        @endif

                        @if($order->shop->address)
                            <div class="col-12">
                                <span class="fw-semibold">Address:</span><br>{{ $order->shop->address }}
                            </div>
                        @endif
                    ...
                    <div class="card-body small">
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item px-0 d-flex justify-content-between">
                                <span class="fw-semibold">Tracking No:</span>
                            ...
                            <tbody>
                                @foreach($order->payments as $payment)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge {{ $payment->status === 'success' ? 'bg-success' : 'bg-secondary' }} px-3 py-2">
            ...
            <div class="card shadow-sm border-0 mt-4">
                ...
                <div class="card-body small">
                    @if($order->order_notes)
                        <p class="mb-2">
                            <span class="fw-semibold">Order Notes:</span><br>
                            {{ $order->order_notes }}
                        </p>
                    @endif
                    @if($order->promo_code)
                        <p class="mb-0">
                            <span class="fw-semibold">Promo Code:</span> {{ $order->promo_code }}
                        </p>
                    @endif
                </div>
            </div>
    ...
    @if(!$item->review)
        ...
        <div class="modal fade" id="{{ $modalId }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <form method="POST"
                      action="{{ route('orders.items.reviews.store', [$item->order_id, $item->id]) }}"
                      class="modal-content">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title">
                            Review – {{ optional($item->product)->name }}
                        </h5>
