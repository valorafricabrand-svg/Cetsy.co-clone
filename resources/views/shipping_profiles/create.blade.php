...
<div class="content">
    ...
    <form method="POST" action="{{ route('shipping_profiles.store') }}">
        @csrf
        ...
        <div class="mb-3">
            ...
            <input type="number" id="base_rate" name="base_rate" min="0" step="0.01"
                class="form-control @error('base_rate') is-invalid @enderror" value="{{ old('base_rate', 0) }}" required>
        ...
        <div class="mb-3">
            ...
            <input type="number" id="delivery_days" name="delivery_days" min="0"
                class="form-control @error('delivery_days') is-invalid @enderror" value="{{ old('delivery_days', 3) }}" required>
            @error('delivery_days') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        <div class="form-check mb-3">
            <input type="checkbox" id="pickup_available" name="pickup_available" class="form-check-input" {{ old('pickup_available') ? 'checked' : '' }}>
