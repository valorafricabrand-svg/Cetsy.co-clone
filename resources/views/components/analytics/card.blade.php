...
<article class="glass analytics-card h-full rounded-2xl border border-slate-200 bg-white p-5 text-center shadow-sm">
    ...
    @if(!is_null($delta))
        @php $up = $delta >= 0; $cls = $up ? 'text-emerald-700' : 'text-rose-700'; @endphp
        <div class="mt-2 text-sm font-semibold {{ $cls }}">
            <i class="fas {{ $up ? 'fa-arrow-up' : 'fa-arrow-down' }} mr-1"></i>{{ number_format($delta, 1) }}%
        </div>
    @endif
    @if($sparkId)
        <div class="mt-4 h-10 w-full rounded-xl border border-slate-200 bg-slate-50 px-2 py-1">
