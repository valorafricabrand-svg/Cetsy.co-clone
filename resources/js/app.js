...
function initLiveNotificationPulse() {
    const pulseMeta = document.querySelector('meta[name="cetsy-notifications-pulse-url"]');
    const pulseUrl = pulseMeta?.content?.trim();
    if (!pulseUrl) {
        return;
    }
    ...
    poll();
    state.pollTimer = window.setInterval(poll, 15000);
    ...
    function ensureSettingsModal() {
                ...
                <div class="rounded-2xl border border-slate-200 bg-white shadow-xl">
                    <div class="flex items-center justify-between border-b border-slate-200 px-4 py-3">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-slate-500">Control sounds, toasts, and browser popups for new messages and sales.</p>
                        </div>
                    ...
                    <div class="space-y-4 px-4 py-4">
                        ...
                        <div class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-3">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    <p class="text-sm font-semibold text-slate-900">Browser popups</p>
        ...
        requestButton?.addEventListener("click", async () => {
            ...
            if (permission === "granted") {
                showToast({
                    ...
                    body: "You will now get browser popups while this app is open in the background.",
                    tone: "general",
                });
            }
        ...
        modal.querySelectorAll("[data-test-live-sound]").forEach((button) => {
            button.addEventListener("click", async () => {
                await unlockAudio();
                const kind = button.getAttribute("data-test-live-sound");
                if (kind === "sale") {
                    playSaleSound();
                } else {
                    playMessageSound();
    ...
    function injectLiveNotificationStyles() {
        ...
        style.textContent = `
            ...
            .live-alert-toast--message .live-alert-toast__icon {
                background: #dbeafe;
                color: #1d4ed8;
            }
            .live-alert-toast--sale .live-alert-toast__icon {
                background: #dcfce7;
                color: #15803d;
            }
            .live-alert-toast--general .live-alert-toast__icon {
                background: #f1f5f9;
                color: #334155;
            }
            .live-alert-toast__body {
                min-width: 0;
                flex: 1;
            }
            .live-alert-toast__title {
                margin: 0;
                font-size: 0.92rem;
                font-weight: 700;
                color: #0f172a;
            }
            .live-alert-toast__text {
                margin: 0.18rem 0 0;
                color: #475569;
                font-size: 0.82rem;
                line-height: 1.45;
            }
            .live-alert-toast__actions {
                margin-top: 0.55rem;
                display: flex;
                align-items: center;
                gap: 0.55rem;
            }
            .live-alert-toast__link {
                display: inline-flex;
                align-items: center;
                justify-content: center;
                border-radius: 9999px;
            ...
            .live-alert-pref__title {
                display: block;
                color: #0f172a;
                font-size: 0.9rem;
                font-weight: 700;
            }
            .live-alert-pref__desc {
                display: block;
                margin-top: 0.18rem;
                color: #64748b;
                font-size: 0.78rem;
            ...
            .live-alert-pref input[type="checkbox"] {
                height: 1.05rem;
                width: 1.05rem;
                flex: 0 0 1.05rem;
            }
